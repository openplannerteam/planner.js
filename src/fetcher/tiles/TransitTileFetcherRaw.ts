...
export default class TransitTileFetcherRaw implements ITransitTileFetcher{
    ...
    public async get(url: string): Promise<TransitTile> {
        ...
        if (response.status === 200 && responseText) {
          ...
          return this.processTileData(url, nodes, ways, relations);
        } else {
          return new TransitTile(url, new Set(), new Set(), new Set());
        }
      }
      ...
      protected processTileData(url: string, nodes: IRoutableTileNodeIndex, ways: IRoutableTileWayIndex, relations: Set<HypermediaTreeRelation>) {
        ...
        return new TransitTile(url, new Set(Object.keys(nodes)), new Set(Object.keys(ways)), relations);
      }
